...
def compute_list_operations_similar(dict1, dict2):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for proposed_question_type, proposed_question_dict in proposed_questions.items():
                ...
                for rephrased_question_hybrid_type, rephrased_question_hybrid_dict in zip(rephrased_questions_indexes_hybrid, rephrased_questions_hybrid):
                    ...
                    for answer_key_name in answer_key_names:
                        ...
                        # if referencing_F1_value < 1:
                        #     print("answer_key_name", answer_key_name)
                            
                        #     # print("positive", positive)
                        #     print("needed_corpusid2senids", needed_corpusid2senids)
                        ...
                        total_score_key_name = f"rephrased_hybrid-{answer_key_name}-consistency_with_sens2-total_score"
                        total_score = sum([len(v) for v in needed_corpusid2senids.values()])
                        rephrased_question_hybrid_dict[total_score_key_name] = total_score      
                        if not args.only_eval_F1 and args.eval_answerable and (score_key_name not in rephrased_question_hybrid_dict or rephrased_question_hybrid_dict[score_key_name] == None):
                            ...
                            # if rephrased_question_hybrid_dict[score_key_name]>total_score:
                            #     print("rephrased_question_hybrid_dict[score_key_name]:", rephrased_question_hybrid_dict[score_key_name])
                            #     print("total_score:", total_score)
                            #     print("positive:", positive)
                            #     print("analysis_process_key_name:", json.dumps(rephrased_question_hybrid_dict[analysis_process_key_name], indent=4))
                            #     # print("generated_answer:", generated_answer)
                            #     input("Press Enter to continue...")
                            #     print()
                        ...
                        if not args.only_eval_F1 and args.eval_partanswerable and (score_key_name not in rephrased_question_hybrid_dict or rephrased_question_hybrid_dict[score_key_name] == None):
                                if judge_criterias:
                                    ...
                                    futures_to_data[future] = (rephrased_question_hybrid_dict, answer_key_name, cur_top_k_value, total_score, 'dont_know_partthing2', analysis_process_key_name, score_key_name)
        ...
        for future in tqdm(as_completed(futures_to_data), total=len(futures_to_data), desc="Processing Futures", dynamic_ncols=True):
            ...
            try:
                ...
                elif score_type == 'dont_know_partthing2':
                    analysis_process, score = extract_consistency_with_sens_output(score_response)
                    proposed_question_dict[analysis_process_key_name] = analysis_process
                    proposed_question_dict[score_key_name] = score
                elif score_type in ['dont_know_anything2']:
                    analysis_process, score = extract_consistency_with_sens_output(score_response)
                    proposed_question_dict[analysis_process_key_name] = analysis_process
                    proposed_question_dict[score_key_name] = score
            ...
            for proposed_question_type, proposed_question_dict in proposed_questions.items():
                ...
                for rephrased_question_hybrid_type, rephrased_question_hybrid_dict in zip(rephrased_questions_indexes_hybrid, rephrased_questions_hybrid):
                    ...
                    for answer_key_name in answer_key_names:
                        ...
                        cur_needed_corpusid2corpus = {k: v for k, v in needed_corpusid2corpus.items()}
                        cur_needed_corpusid2senids = {k: v for k, v in needed_corpusid2senids.items()}
        ...
        for future in tqdm(as_completed(futures_to_data), total=len(futures_to_data), desc="Processing Futures", dynamic_ncols=True):
            ...
            try:
                ...
                if score_type in ['consistency_with_sens2']:
                    ...
                    proposed_question_dict[analysis_process_key_name] = analysis_process
                    proposed_question_dict[score_key_name] = score
                elif score_type in ['dont_know_partthing2']:
                    analysis_process, score = extract_consistency_with_sens_output(score_response)
                    proposed_question_dict[analysis_process_key_name] = analysis_process
                    proposed_question_dict[score_key_name] = score
                elif score_type in ['dont_know_anything2']:
