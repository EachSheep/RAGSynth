...
PREPROCESS_ENV_PATH="admission.stanford.edu.filter.train/admission.stanford.edu.filter.preprocess.env"

# Validate propose_type and set postprocess_env_path
case "$PROPOSE_TYPE" in  
  content)  
    echo "Valid propose_type: $PROPOSE_TYPE"
    POSTPROCESS_ENV_PATH="admission.stanford.edu.filter.train/admission.stanford.edu.filter.postprocess.content.env"
    
    # Execute the python script with the preprocess environment path for content
    python query_generation_train.py \
  ...
  entity_graph)
    echo "Valid propose_type: $PROPOSE_TYPE"
    POSTPROCESS_ENV_PATH="admission.stanford.edu.filter.train/admission.stanford.edu.filter.postprocess.entitygraph.env"
    ;;
  *)  
