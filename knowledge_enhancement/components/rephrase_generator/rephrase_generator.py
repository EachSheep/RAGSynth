...
class RephraseGenerator:
    def __init__(self, save_interval=20):
        ...
        self.REPHRASE_GENERATOR_INPUT_PATH, self.REPHRASE_GENERATOR_PROMPT_PATH, self.REPHRASE_GENERATOR_OUTPUT_PATH = None, None, None
        if os.getenv("REPHRASE_GENERATOR_CONTENT_INPUT_PATH", None) != None:
            self.REPHRASE_GENERATOR_INPUT_PATH = os.getenv("REPHRASE_GENERATOR_CONTENT_INPUT_PATH")
            self.REPHRASE_GENERATOR_PROMPT_PATH = os.getenv("REPHRASE_GENERATOR_CONTENT_PROMPT_PATH")
            self.REPHRASE_GENERATOR_OUTPUT_PATH = os.getenv("REPHRASE_GENERATOR_CONTENT_OUTPUT_PATH")
            self.REPHRASE_GENERATOR_GENERATED_TYPE = 'content'
        elif os.getenv("REPHRASE_GENERATOR_ENTITYGRAPH_INPUT_PATH", None) != None:
            self.REPHRASE_GENERATOR_INPUT_PATH = os.getenv("REPHRASE_GENERATOR_ENTITYGRAPH_INPUT_PATH")
            self.REPHRASE_GENERATOR_PROMPT_PATH = os.getenv("REPHRASE_GENERATOR_ENTITYGRAPH_PROMPT_PATH")
            self.REPHRASE_GENERATOR_OUTPUT_PATH = os.getenv("REPHRASE_GENERATOR_ENTITYGRAPH_OUTPUT_PATH")
            self.REPHRASE_GENERATOR_GENERATED_TYPE = 'entity_graph'
        else:
            raise EnvironmentError("Environment variable 'REPHRASE_GENERATOR_CONTENT_INPUT_PATH' or 'REPHRASE_GENERATOR_ENTITYGRAPH_INPUT_PATH' is not set.")
        
        self.REPHRASE_GENERATOR_INPUT_PATH = os.path.join(CUSTOM_CORPUS_HOME, self.REPHRASE_GENERATOR_INPUT_PATH)
        self.REPHRASE_GENERATOR_PROMPT_PATH = os.path.join(CUSTOM_CORPUS_HOME, self.REPHRASE_GENERATOR_PROMPT_PATH)
        self.REPHRASE_GENERATOR_OUTPUT_PATH = os.path.join(CUSTOM_CORPUS_HOME, self.REPHRASE_GENERATOR_OUTPUT_PATH)
        ...
        if os.path.exists(self.REPHRASE_GENERATOR_OUTPUT_PATH):
            with open(self.REPHRASE_GENERATOR_OUTPUT_PATH, "r", encoding="utf-8") as f:
                self.inputs = json.load(f)
            print(f"Loaded rephrase generator {len(self.inputs)} examples from {os.path.relpath(self.REPHRASE_GENERATOR_OUTPUT_PATH, CUSTOM_CORPUS_HOME)}.")
        else:
            with open(self.REPHRASE_GENERATOR_INPUT_PATH, "r", encoding="utf-8") as f:
                self.inputs = json.load(f)
            print(f"Loaded rephrase generator {len(self.inputs)} examples from {os.path.relpath(self.REPHRASE_GENERATOR_INPUT_PATH, CUSTOM_CORPUS_HOME)}.")
    ...
    def run(self):
        ...
        with open(self.REPHRASE_GENERATOR_PROMPT_PATH, "r", encoding="utf-8") as f:
            rephrase_generator_prompt = f.read()
        print(f"Loaded rephrase generator prompt from {os.path.relpath(self.REPHRASE_GENERATOR_PROMPT_PATH, CUSTOM_CORPUS_HOME)}.")

        all_num, success_num = 0, 0
        tasks = []

        with ThreadPoolExecutor(max_workers=self.REPHRASE_GENERATOR_NUM_WORKERS) as executor:
            if self.REPHRASE_GENERATOR_GENERATED_TYPE in ['content']:
                for i, cur_input in enumerate(self.inputs[:self.REPHRASE_GENERATOR_MAX_GEN_TIMES]):
                    ...
                    for proposed_question_type, proposed_question_dict in questions.items():
                        if 'rephrased-questions' in proposed_question_dict and proposed_question_dict['rephrased-questions']:
                            continue
            ...
            elif self.REPHRASE_GENERATOR_GENERATED_TYPE in ['entity_graph']:
                ...
                raise ValueError(f"Invalid value for 'REPHRASE_GENERATOR_GENERATED_TYPE': {self.REPHRASE_GENERATOR_GENERATED_TYPE}")
            ...
            for future_info in tqdm(as_completed([t[0] for t in tasks]), total=len(tasks), desc="Generating", dynamic_ncols=True):
                ...
                try:
                    ...
                    if rephrased_questions:
                        proposed_question_dict['rephrased-questions'] = rephrased_questions  # inplace update
                        success_num += 1
                        if success_num % self.save_interval == 0:
                            dir_path = os.path.dirname(self.REPHRASE_GENERATOR_OUTPUT_PATH)
                            os.makedirs(dir_path, exist_ok=True)
                            print(f'Saving {success_num}/{all_num} outputs to {os.path.relpath(self.REPHRASE_GENERATOR_OUTPUT_PATH, CUSTOM_CORPUS_HOME)}.')
                            with open(self.REPHRASE_GENERATOR_OUTPUT_PATH, 'w', encoding="utf-8") as f:
                                json.dump(self.inputs, f, indent=2, ensure_ascii=False)
                except Exception as e:
                    print(f"Error processing question: {e}")

        if success_num or not os.path.exists(self.REPHRASE_GENERATOR_OUTPUT_PATH):
            dir_path = os.path.dirname(self.REPHRASE_GENERATOR_OUTPUT_PATH)
            os.makedirs(dir_path, exist_ok=True)
            print(f'Saving {success_num}/{all_num} outputs to {os.path.relpath(self.REPHRASE_GENERATOR_OUTPUT_PATH, CUSTOM_CORPUS_HOME)}.')
            with open(self.REPHRASE_GENERATOR_OUTPUT_PATH, 'w', encoding="utf-8") as f:
