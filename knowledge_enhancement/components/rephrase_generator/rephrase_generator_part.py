...
class RephraseGeneratorPart:
    def __init__(self, save_interval=20):
        ...
        self.REPHRASE_GENERATOR_PART_INPUT_PATH = os.path.join(CUSTOM_CORPUS_HOME, self.REPHRASE_GENERATOR_PART_INPUT_PATH)
        self.REPHRASE_GENERATOR_PART_PROMPT_PATH = os.path.join(CUSTOM_CORPUS_HOME, self.REPHRASE_GENERATOR_PART_PROMPT_PATH)
        self.REPHRASE_GENERATOR_PART_OUTPUT_PATH = os.path.join(CUSTOM_CORPUS_HOME, self.REPHRASE_GENERATOR_PART_OUTPUT_PATH)
        ...
        if os.path.exists(self.REPHRASE_GENERATOR_PART_OUTPUT_PATH):
            with open(self.REPHRASE_GENERATOR_PART_OUTPUT_PATH, "r", encoding="utf-8") as f:
                self.inputs = json.load(f)
            print(f"Loaded rephrase generator {len(self.inputs)} examples from {os.path.relpath(self.REPHRASE_GENERATOR_PART_OUTPUT_PATH, CUSTOM_CORPUS_HOME)}.")
        else:
            with open(self.REPHRASE_GENERATOR_PART_INPUT_PATH, "r", encoding="utf-8") as f:
                self.inputs = json.load(f)
            print(f"Loaded rephrase generator {len(self.inputs)} examples from {os.path.relpath(self.REPHRASE_GENERATOR_PART_INPUT_PATH, CUSTOM_CORPUS_HOME)}.")
    ...
    def run(self):
        ...
        with open(self.REPHRASE_GENERATOR_PART_PROMPT_PATH, "r", encoding="utf-8") as f:
            rephrase_generator_prompt = f.read()
        print(f"Loaded rephrase generator prompt from {os.path.relpath(self.REPHRASE_GENERATOR_PART_PROMPT_PATH, CUSTOM_CORPUS_HOME)}.")
                ...
                try:
                    ...
                    if rephrased_questions:
                        ...
                        if success_num % self.save_interval == 0:
                            dir_path = os.path.dirname(self.REPHRASE_GENERATOR_PART_OUTPUT_PATH)
                            os.makedirs(dir_path, exist_ok=True)
                            print(f'Saving {success_num}/{all_num} outputs to {os.path.relpath(self.REPHRASE_GENERATOR_PART_OUTPUT_PATH, CUSTOM_CORPUS_HOME)}.')
                            with open(self.REPHRASE_GENERATOR_PART_OUTPUT_PATH, 'w', encoding="utf-8") as f:
